...
import {parseControllerHandshake, parseHeaterControl} from '../util.ts'
        ...
        data(socket, buffer) {
            ...
            switch (messageType) {
                ...
                case MessageType.SENSOR_READING:
                    // 0x03 messages: [type, temp, unknown, frequencySeconds]
                    eventBus.emit(
                ...
                case MessageType.LOCAL_HEATER_CONTROL:
                    // 0x07 messages: sent by heater after on/off commands
                    eventBus.emit(HuumEvents.LOCAL_HEATER_CONTROL, parseHeaterControl(buffer))
                    break
